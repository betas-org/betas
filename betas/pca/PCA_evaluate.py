...
def get_misclassification_error(features, labels, beta):
    ...
    error = 1 - accuracy_score(labels, labels_pred)
    return error
...
def run_pca_across_dimensions(train_features_std, train_labels,
    ...
    for dimension in dimensions:
        ...
        lr_model = LogisticRegression(penalty='l2', tol=0.001,
        ...
        missclassification_err_train[i] = \
            get_misclassification_error(train_features_pca_std, train_labels, beta_values)
                                              
        missclassification_err_test[i] = \
            get_misclassification_error(test_features_pca_std,test_labels, beta_values)
        i += 1

    return missclassification_err_train, missclassification_err_test, dimensions
...
def plot_pca_errors(missclassification_err_train, missclassification_err_test,
    ...
    ax1.set_xlabel('PCA Dimensions', fontsize=10)
    ax1.set_xticks(dimensions)
    color = 'tab:red'
    ax1.set_ylabel('Training Misclassification Error', fontsize=10, color=color)
    ...
    ax2.set_ylabel('Test Misclassification Error', fontsize=10, color=color)
